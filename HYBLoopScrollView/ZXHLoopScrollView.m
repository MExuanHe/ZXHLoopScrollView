 ...
 */
@interface ZXHCollectionCell : UICollectionViewCell

@property (nonatomic, strong) ZXHLoadImageView *imageView;
@property (nonatomic, strong) UILabel          *titleLabel;
@property (nonatomic, assign) BOOL             isDragging;

@end

@implementation ZXHCollectionCell

- (instancetype)initWithFrame:(CGRect)frame {
  if (self = [super initWithFrame:frame]) {
    self.imageView = [[ZXHLoadImageView alloc] init];
...
- (void)layoutSubviews {
  ...
  self.titleLabel.frame = CGRectMake(0, self.ZXH_height - 30, self.ZXH_width, 30);
  self.titleLabel.hidden = self.titleLabel.text.length > 0 ? NO : YES;
}

@end

@interface ZXHLoopScrollView () <UICollectionViewDataSource, UICollectionViewDelegate> {
  ZXHPageControl *_pageControl;
}

@property (nonatomic, copy) ZXHLoopScrollViewDidSelectItemBlock didSelectItemBlock;
@property (nonatomic, copy) ZXHLoopScrollViewDidScrollBlock didScrollBlock;
...
@implementation ZXHLoopScrollView

- (void)dealloc {
  //  NSLog(@"ZXHloopscrollview dealloc");
...
+ (instancetype)loopScrollViewWithFrame:(CGRect)frame
  ...
  ZXHLoopScrollView *loopView = [[ZXHLoopScrollView alloc] initWithFrame:frame];
...
- (instancetype)initWithFrame:(CGRect)frame {
  if (self = [super initWithFrame:frame]) {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:[UIApplication sharedApplication]
                                             selector:NSSelectorFromString(@"ZXH_clearCache")
                                                 name:UIApplicationDidReceiveMemoryWarningNotification
                                               object:nil];
  }
  return self;
...
- (void)configCollectionView {
  ...
  [self.collectionView  registerClass:[ZXHCollectionCell class]
...
- (void)configPageControl {
  if (self.pageControl == nil) {
    _pageControl = [[ZXHPageControl alloc] init];
  ...
  self.pageControl.ZXH_size = size;
  
  if (self.alignment == kPageControlAlignCenter) {
    self.pageControl.ZXH_originX = (self.ZXH_width - self.pageControl.ZXH_width) / 2.0;
  } else if (self.alignment == kPageControlAlignRight) {
    self.pageControl.ZXH_rightX = self.ZXH_width;
  }
  self.pageControl.ZXH_originY = self.ZXH_height - self.pageControl.ZXH_height + 5;
}
...
- (void)layoutSubviews {
  ...
  self.layout.itemSize = self.ZXH_size;
...
- (void)setAlignment:(ZXHPageControlAlignment)alignment {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
  ...
  ZXHCollectionCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:kCellIdentifier
                                                                      forIndexPath:indexPath];
  
  // 先取消之前的请求
  ZXHLoadImageView *preImageView = cell.imageView;
  ...
  if ([preImageView isKindOfClass:[ZXHLoadImageView class]]) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
  ...
  int itemIndex = (scrollView.contentOffset.x +
                   self.collectionView.ZXH_width * 0.5) / self.collectionView.ZXH_width;
  ...
  CGFloat x = scrollView.contentOffset.x - self.collectionView.ZXH_width;
  NSUInteger index = fabs(x) / self.collectionView.ZXH_width;
  CGFloat fIndex = fabs(x) / self.collectionView.ZXH_width;
...
- (void)clearImagesCache {
  // 放在了非公开的扩展中，但是方法是存在的
  if ([[UIApplication sharedApplication] respondsToSelector:NSSelectorFromString(@"ZXH_clearDiskCaches")]) {
    ((void (*)(id, SEL))objc_msgSend)([UIApplication sharedApplication],
                                      NSSelectorFromString(@"ZXH_clearDiskCaches"));
  }
}

- (unsigned long long)imagesCacheSize {
  NSString *directoryPath = [NSHomeDirectory() stringByAppendingString:@"/Documents/ZXHLoopScollViewImages"];
  BOOL isDir = NO;
